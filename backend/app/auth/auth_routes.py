...
def login():
    ...
    return jsonify({"access_token": access_token, "user_id": str(user["_id"]), "message": "Login successful", "is_verified": True}), 200
...
def logout():
    ...
    # Storing JTI in Redis with an expiration time
...
def is_token_revoked(jwt_header, jwt_payload: dict) -> bool:
    ...
    """
    jti = jwt_payload["jti"]
    # Check Redis for the token's JTI
